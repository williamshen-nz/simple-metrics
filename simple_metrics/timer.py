...
from simple_metrics.metrics import TimeMetric
from simple_metrics.metrics_logger import metrics_logger
...
class Timer(object):
    ...
    def start(self) -> "Timer":
        if self._last_start_time:
            ...
            raise RuntimeError(f"Cannot start timer {self.name} that has been stopped!")
        else:
...
def timed(name: str, context: Optional[Dict[str, str]] = None, log_level=logging.INFO):
